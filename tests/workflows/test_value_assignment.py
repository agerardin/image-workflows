...
def test_assign_int(default_input_model: dict[Any, Any]):
    ...
    assert input.type.is_value_assignable(4), f"Was expecting a int, got {input.type}"
    assert not input.type.is_value_assignable("ok")
...
def test_assign_string(default_input_model: dict[Any, Any]):
    ...
    assert input.type.is_value_assignable("ok"), f"Was expecting a int, got {input.type}"
    assert not input.type.is_value_assignable(4)
...
def test_assign_array_of_strings(default_input_model: dict[Any, Any]):
    ...
    assert input.type.is_value_assignable(["ok"])
    assert input.type.is_value_assignable(["ok1","ok2"])
    assert not input.type.is_value_assignable(["ok", 4]) #cannot mix strings and ints
...
def test_assign_array_of_files(default_input_model: dict[Any, Any]):
    ...
    assert input.type.is_value_assignable(
...
def test_assign_nested_array(default_input_model: dict[Any, Any]):
    ...
    assert input.type.is_value_assignable([["ok"]]) # nested array of strings ok
    assert not input.type.is_value_assignable([[4]]) # nested array of int not ok
    assert not input.type.is_value_assignable([4]) # simple array not ok
    assert not input.type.is_value_assignable([[["ok"]]]) # deeper array nesting not ok
    assert not input.type.is_value_assignable([["ok"],[4]]) # cannot mix and match types.
