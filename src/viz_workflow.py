...
def _rewrite_bbbcdowload_outdir_for_cwl(cwl_workflow : Workflow, config):
    """
    TODO REMOVE. Every attempt to get this done has been a failure.
    Either generate two workflows (download and the rest) with matching
    outDir and inpDir, or wait for wic update fix.
    Rewrite cwlJobInputs.bbbcdownload___outDir.path with its final value as
    a workaround of how wic create explicit link between step params.
    ...
    inputs_path = WIC_PATH / "autogenerated" / (cwl_workflow.name + "_inputs.yml")
    print(inputs_path)
    inputs = utils.load_yaml(inputs_path)
    out_dir = config['outputs']['Bbbcdownload.outDir']['path']
    current_path = inputs['viz_workflow_BBBC001__step__1__BbbcDownload___outDir']['location']
    inputs['viz_workflow_BBBC001__step__1__BbbcDownload___outDir']['location'] = (Path(current_path) / out_dir).as_posix()
    utils.save_yaml(inputs, inputs_path)
...
def _convert_to_compute_workflow(
    ...
    for compute_step_name in compute["steps"]:
        ...
        add_step_run_cwl_name(compute["steps"][compute_step_name], compute_step_name)
...
def translate_wic_names_to_valid_argo_names(compute):
...
def update_paths_for_compute(cwlJobInputs):
    """
        ...
        paths. For that we prepend the mounted volume path for each argo job.
    """
    # check first which attribute we need to udpdate
    if(COMPUTE_COMPATIBILITY):
        directory_attr = "path"
    else: 
        directory_attr = "location"
...
def add_step_run_cwl_name(compute_step, compute_step_name):
    """
...
if __name__ == "__main__":
    ...
    # TODO REMOVE those modifications should occur on the driver side
