...
def viz_workflow(dataset_name : str, 
    ...
    if RUN_LOCAL:
        ...
        _rewrite_bbbcdowload_outdir(compute_workflow, config)
...
def _rewrite_bbbcdowload_outdir(compute_workflow, config):
    """
    ...
    out_dir = config['outputs']['bbbcdownload.outDir'][directory_attr]
    
    try:
        path = compute_workflow['cwlJobInputs']['bbbcdownload___outDir']['path']
        current_path = Path(path)
        compute_workflow['cwlJobInputs']['bbbcdownload___outDir']['path'] = (current_path / out_dir).as_posix()
    except KeyError:
...
if __name__ == "__main__":
    ...
    viz_workflow(dataset_name=dataset_name,
                 ...
                 wic_path = WIC_PATH,
                 compute_path = COMPUTE_SPEC_PATH,
                 download=False,
                 convert=False,
                 montage=False,
